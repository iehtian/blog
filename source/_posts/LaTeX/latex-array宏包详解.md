...
在 LaTeX 中，`tabular` 和 `array` 环境是排版表格的核心工具。然而，标准 LaTeX 提供的列格式选项比较有限——只有 `l`（左对齐）、`c`（居中对齐）、`r`（右对齐）和 `p`（段落列）四种。`array` 宏包极大地扩展了列格式的自定义能力，允许你在列前后插入命令、定义全新的列类型、控制列宽等，是每个 LaTeX 表格使用者都应该掌握的基础宏包。
...
注意：`array` 宏包会同时增强 `tabular`（文本模式）和 `array`（数学模式）两个环境。加载后，两个环境都享有本文介绍的全部列格式功能。
...
`array` 宏包在此基础上新增了 `m` 和 `b` 列类型，以及自定义列类型的能力。

## 4. 新增的固定宽度列类型

### 4.1 `m{宽度}` — 垂直居中对齐

`p{宽度}` 的缺点是：当相邻列高度不一致时，`p` 列的内容默认顶部对齐，视觉上不够协调。`m{宽度}` 将段落内容的垂直参考线放在段落的正中间：
...
在 `|p{2cm}|m{2cm}|` 的对比中，`p` 列内容顶部对齐而 `m` 列内容垂直居中，差异一目了然。
...
第二列内容来自数值，第一列结尾自动添加了句点。也可以用在数学符号场景中：
...
第三列每个单元格自动追加 `^\circ`（度符号），显示为 `90°`、`45°`。

**提示**：`<{...}` 中的命令出现在该列内容之后、下一列的 `>{...}` 命令之前。
...
使用 `E{\bfseries}{c}` 创建一个自动加粗的居中对齐列——它等价于 `>{\bfseries}c<{\bfseries}`。当然这个设计比较刻意，一般不会这样用，但它展示了 `\newcolumntype` 的灵活性。

### 6.3 `\arraybackslash` 的作用
...
这是新手最常遇到的坑之一。养成习惯：只要 `>{...}` 里出现 `\centering`、`\raggedright` 或 `\raggedleft`，就追加 `\arraybackslash`。

## 7. 控制列间距

### 7.1 `\setlength` 调整表格列间距
...
这是标准 LaTeX 就支持的语法，与 `array` 宏包配合使用时更加灵活。

### 7.3 `!{}` 修饰符

标准 LaTeX 中，`@{}` 会移除原有的列间距。而在某些情况下，你只希望**额外插入**内容而**不**移除原有间距，这时可用 `!{}`：
...
## 11. booktabs 宏包 — 专业表格线

### 11.1 为什么需要 booktabs

标准 LaTeX 的 `\hline` 产生的横线粗细一致，且竖线与横线交叉处容易产生缝隙，不够美观。`booktabs` 宏包提供了三种不同粗细的专业表格线，遵循学术出版中"三线表"的排版规范：

- **不使用竖线**：专业表格通常不依赖竖线来区分列，而是通过合理的列间距和对齐来实现
- **横线有粗细层次**：顶线和底线较粗，中间分隔线较细，形成清晰的视觉层次
- **线与内容之间有适当间距**：防止文字与横线贴得过紧

`booktabs` 的核心理念来自 Simon Fear 在 1992 年发表的论文 *Publication quality tables in LaTeX*，至今仍是 LaTeX 学术表格排版的事实标准。
...
`booktabs` 与 `array` 宏包完全兼容，通常同时加载：
...
缩短的目的是让同一条水平线上的多条 `\cmidrule` 之间保留微小间隙，视觉上能区分不同的列分组。

### 11.5 `\addlinespace` — 增加行间距
...
### 11.8 与 array 配合：最佳实践

`booktabs` 与 `array` 宏包配合是 LaTeX 表格排版的标准组合——`array` 负责列格式的灵活定制，`booktabs` 负责表格线的专业呈现：
...
- **不要混用竖线和 `booktabs`**：`booktabs` 的设计理念是不使用竖线。在 `{|c|c|}` 中使用 `\toprule` 虽然能编译，但视觉效果不理想，竖线与横线粗细反差过大。
...
- **间距由宏包管理**：`\aboverulesep` 和 `\belowrulesep` 已经自动管理了线与内容之间的间距，通常无需额外手动添加空白。
...
- **编译兼容性**：`array` 宏包与绝大多数 LaTeX 宏包兼容（`booktabs`、`longtable`、`tabularx`、`xcolor` 等），且这些宏包很多本身就会自动加载 `array`。
